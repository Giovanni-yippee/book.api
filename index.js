...
app.post('/api/register', (req, res) => {
    ...
    if (!username || !password) {
        return res.status(401).json({ error: 'Атыңыз немесе пароль енгізілмеді' });
    }
    ...
    res.status(201).json({ message: 'Пайдаланушы сәтті тіркелді', user: newUser });
});
...
app.post('/api/login', (req, res) => {
    ...
    if (!user) {
        return res.status(401).json({ error: 'Атыңыз немесе пароль қате' });
    }

    res.json({ message: 'Сіз сәтті кірдіңіз', user });
});
